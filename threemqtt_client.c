...
#define ADDRESS     "tcp://192.168.29.210:1883"
#define CLIENTID    "CClient"
#define TOPIC       "test/topic"
#define RESPONSE_TOPIC "test/response_c"
...
int onMessageArrived(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    } else if (strcmp(topicName, RESPONSE_TOPIC) == 0) {
        printf("C Client: Received message from Broker: %.*s\n", message->payloadlen, (char*)message->payload);
